...
GazeboRosOpenniKinect::GazeboRosOpenniKinect()
{
  ...
  ROS_INFO_STREAM("Kinect plugin[BRASS] loaded");
}
...
void GazeboRosOpenniKinect::Advertise()
{
  ...
  ROS_INFO_STREAM("Subscribing to /sensor/kinect/onoff");
...
void GazeboRosOpenniKinect::OnNewImageFrame(const unsigned char *_image,
    ...
    const std::string &_format)
{
  if (!this->initialized_ || this->height_ <=0 || this->width_ <=0 || !this->sensor_on_)
    return;
...
void GazeboRosOpenniKinect::sensorOnOffCallback(const std_msgs::String::ConstPtr& msg) {
  ROS_INFO_STREAM("Kinect: " << msg->data.c_str());
  // if (msg->data.c_str () == "on") {
  if (strcmp(msg->data.c_str(), "on")==0) {
    ROS_INFO_STREAM("on");
    this->sensor_on_ = true;
  }
  else if (strcmp(msg->data.c_str(), "off")==0) {
  // else if (msg->data.c_str()=="off") {
    ROS_INFO_STREAM("off");
